...
private:  
    ...
    pthread_t *all_threads;   //线程数组  //pthread_t用于声明线程ID。
...
{  
    ...
    if(pthread_create(all_threads + i, NULL, worker, this) != 0)  //pthread_create是UNIX环境创建线程函数,第一个参数为指向线程标识符的指针。//第二个参数用来设置线程属性。//第三个参数是线程运行函数的地址。//最后一个参数是运行函数的参数。
