...
char *path = "/home/ys/Web/WebServer"; //路径  
...
public:  
    ...
    {  
        char buf[512];  
        sprintf(buf, "HTTP/1.1 %d OK\r\nConnection: Close\r\n"  //响应头  //发送格式化输出到 str 所指向的字符串。
        "content-length:%d\r\n\r\n", status, strlen(message));  
  
        sprintf(buf, "%s%s", buf, message);  
        write(connfd, buf, strlen(buf));  //write()会把参数buf所指的内存写入count个字节到参数fd所指的文件内。
...
{  
    ...
    int size;  
    size = read(connfd, buffer, BUFFER_SIZE - 1);  //读取Http请求报文  //read()会把参数fd所指的文件传送count 个字节到buf 指针所指的内存中。
    //printf("%s", buffer);  
    if(size > 0)  
    {  
        char method[5];  
        ...
        if(strcasecmp(method, "GET") == 0)  //get method  //strcasecmp判断字符串是否相等(忽略大小写)
        {  
        ...
        {  
            //printf("Begin\n");  
            char argvs[100];  
            memset(argvs, 0, sizeof(argvs));  //这个函数在socket中多用于清空数组,Memset 用来对一段内存空间全部设置为某个字符，一般用在对定义的字符串进行初始化为‘ ’或‘/0’；
            int k = 0;  
            char *ch = NULL;  
            ++j;  
            while((ch = strstr(argvs, "Content-Length")) == NULL) //查找请求头部中的Content-Length行  //函数用于判断字符串str2是否是str1的子串。如果是，则该函数返回str2在str1中首次出现的地址；否则，返回NULL。
            {  
                k = 0;  
